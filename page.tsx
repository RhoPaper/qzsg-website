...
import 'styles/animations.css' // 新增：引入外部样式表
  ...
  return (
    <div className="min-h-screen">
      <Head>
        ...
        <link rel="icon" href="/favicon.ico" /> {/* 新增：添加 favicon */}
      </Head>
      ...
      >
        <div className="container mx-auto flex items-center justify-between p-4">
          <Link href="/" className="flex items-center gap-2"> {/* 修改：将 div 替换为 Link */}
            <Image
              src="/images/logo.png" // 修改：更新 logo 路径
              alt="亲子拾光"
              width={150} // 修改：更新 logo 宽度
              height={40} // 修改：更新 logo 高度
      ...
      <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
        <div
          className="absolute inset-0 bg-[url('/图片/主页背景')] bg-cover bg-center opacity-50" // 修改：更新背景图片路径
          style={{
          ...
          <div className="relative w-full max-w-3xl mx-auto aspect-square">
            ...
            {[0, 1, 2, 3, 4, 5].map((index) => (
              <AnimatedSection
                key={index}
                delay={index * 200}
                className="absolute w-24 h-24 rotating-avatar" // 修改：添加 rotating-avatar 类
                style={{
              ...
              >
                <div className="w-full h-full rounded-full bg-white shadow-lg overflow-hidden transform hover:scale-110 transition-transform duration-300">
                  <Image
                    src={`/图片/导师${index + 1}`} // 修改：更新导师头像图片路径
                    alt={`导师 ${index + 1}`}
                    width={96}
                    height={96}
